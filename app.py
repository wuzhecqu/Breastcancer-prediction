...
# 特征类型：正系数增加恶性风险，负系数降低恶性风险
feature_directions = {
    'radius_worst': '正相关',
    'concave points_mean': '正相关', 
    'radius_se': '正相关',
    'concavity_worst': '正相关',
    'area_worst': '负相关',
    'compactness_mean': '负相关'
}
            ...
            try:
                ...
                try:
                    ...
                    **力力图解读**:
                    ...
                    st.warning(f"力力图生成失败: {e}")
                    st.info("SHAP条形图已提供完整的特征影响分析")
                ...
                with col_interpret1:
                    ...
                    if not top_risk.empty:
                        for _, row in top_risk.iterrows():
                            direction = "正相关" if feature_directions[row['特征']] == '正相关' else "负相关"
                            st.markdown(f"- **{row['特征']}** = {row['特征值']:.3f}")
                            st.markdown(f"  - SHAP值: +{row['SHAP值']:.4f}")
                            st.markdown(f"  - 与恶性{direction}")
                    else:
                        st.info("无显著风险因素")
...
# ====================== 4. 特征分析页面 ======================
    ...
    tab1, tab2, tab3 = st.tabs(["📈 特征重要性", "📊 特征分布", "ℹ️ 特征说明"])
    ...
    with tab1:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            st.markdown("""
            **特征重要性解读**:
            - 重要性值越高，该特征对模型决策的影响越大
            - 特征重要性基于模型在训练过程中的分裂增益计算
            - 这反映了特征在整个数据集上的全局重要性
            """)
        else:
            st.info("该模型不提供特征重要性属性")
    
    with tab2:
        st.subheader("特征相关性分析")
        
        # 创建特征矩阵（示例数据）
        st.markdown("6个关键特征的相关性矩阵:")
        
        # 生成示例数据
        np.random.seed(42)
        n_samples = 100
        example_data = {
            'radius_worst': np.random.normal(16, 4, n_samples),
            'concave points_mean': np.random.normal(0.06, 0.02, n_samples),
            'radius_se': np.random.normal(0.5, 0.2, n_samples),
            'concavity_worst': np.random.normal(0.15, 0.05, n_samples),
            'area_worst': np.random.normal(900, 300, n_samples),
            'compactness_mean': np.random.normal(0.15, 0.04, n_samples)
        }
    ...
    with tab3:
        ...
        feature_info_df = pd.DataFrame({
            '特征': selected_features,
            '描述': [feature_descriptions.get(f, '暂无描述') for f in selected_features],
            '与恶性的关系': [feature_directions.get(f, '未知') for f in selected_features],
            '典型范围': [
                '6.0-36.0',
                '0.0-0.2', 
                '0.1-3.0',
        ...
        1. **radius_worst (最差半径)**
           - **临床意义**: 肿瘤最大截面的半径，是最重要的恶性指标
           - **恶性特征**: 值越大，恶性可能性越高
           - **参考范围**: 良性通常 < 15，恶性通常 > 20
        
        2. **concave points_mean (平均凹点数量)**
           - **临床意义**: 细胞核轮廓中凹点的平均数量
           - **恶性特征**: 凹点越多、越深，恶性可能性越大
           - **生物学解释**: 恶性细胞核通常形状不规则
...
# ====================== 5. 使用说明页面 ======================
    ...
    | 特征 | 类型 | 临床意义 |
    |------|------|----------|
    | `radius_worst` | 正相关 | 肿瘤最差半径，最重要的恶性指标 |
    | `concave points_mean` | 正相关 | 平均凹点数量，凹点越多恶性可能越大 |
    | `radius_se` | 正相关 | 半径标准误，反映细胞大小变化 |
    | `concavity_worst` | 正相关 | 最差凹度，凹度越大恶性可能越高 |
    | `area_worst` | 负相关 | 最差面积，在某些情况下与良性相关 |
    | `compactness_mean` | 负相关 | 平均紧致度，与细胞形状规则性相关 |
